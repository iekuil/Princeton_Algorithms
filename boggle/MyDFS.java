...
public class MyDFS {
    ...
    private final TreeSet<Integer> marked;

    // 需要传入MtTrieST作为参数

    public MyDFS(SET<String> res) {
        if (res == null) {
    ...
    public void process(char[] board, ArrayList<Bag<Integer>> adj, MyTrieST dictionary,
                        int src) {
        ...
        String emptyPrefix = "";
        dfs(board, adj, dictionary, emptyPrefix, src);

    }

    // DFS：从一点出发，对该点的所有邻接节点进行tries前缀查询
    private void dfs(char[] board, ArrayList<Bag<Integer>> adj, MyTrieST dict, String prefix,
                     int v) {
        if (prefix == null) {
            throw new IllegalArgumentException("");
        }
        ...
        for (int w : adj.get(v)) {
            if (!marked.contains(w)) {
                dfs(board, adj, subDict, current, w);
            }
        }
        marked.remove(v);
